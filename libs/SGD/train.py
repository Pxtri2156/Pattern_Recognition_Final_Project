...
def train(args, cfgs):
    ...
    if dev_features == []:
        print("[INFO]:  Split data with train - val : {}-{}".format( 1- cfgs.SGD.SPLIT, cfgs.SGD.SPLIT ))
        X_train , X_val, Y_train, Y_val = train_test_split(train_features, train_labels, test_size= cfgs.SGD.SPLIT, shuffle=True)
    else:
    ...
    model =  SGDClassifier(loss = cfgs.SGD.LOSS, penalty=cfgs.SGD.PENALTY)
    # Training
...
def print_args_cfg(args, cfgs):
    ...
    print("{} Split dataset train - val : {}-{}".format('#'*3, 1- cfgs.SGD.SPLIT, cfgs.SGD.SPLIT ))
    print("{}\n".format('_'*100))
