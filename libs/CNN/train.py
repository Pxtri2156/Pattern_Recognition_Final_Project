...
def arg_parser():
    ...
    parser.add_argument('--argu', '-a', action='store_true', 
                    help='Augment data')
    parser.add_argument('--weight_model', '-o', required=True, 
                        type=str, help='The path will save model')
    parser.add_argument('--architecture_model', '-ar', default="", 
                        type=str, help='The path will save model')
    parser.add_argument('--visualize', '-v', default="", 
                        type=str, help='The path will save model')
...
def print_args(args):
    ...
    print("{} Augment data: \n\t{}".format('#'*3, args.argu))
    print("{} Dataset name to train: \n\t{}".format('#'*3, args.data_n))
    print("{} The path of weight model : \n\t{}".format('#'*3, args.weight_model))
    print("{} The path of architecture model : \n\t{}".format('#'*3, args.architecture_model))
    print("{} The path of visualize : \n\t{}".format('#'*3, args.visualize))
...
def print_cfgs(cfgs):
    ...
    print("{} VISUALIZE_PATH: ".format('\t', cfgs.CNN.VISUALIZE_PATH ))
    print("{} MODEL: ".format('\t' ))
...
def setup_config(cfgs, args):
    ...
    cfgs.ARGUMENT = args.argu
    cfgs.WEIGHTS_PATH = args.weight_model
    if args.architecture_model != "":
        cfgs.ARCHITECTURE_MODEL_PATH = args.architecture_model
    if args.visualize != "":
        cfgs.VISUALIZE_PATH = args.visualize
